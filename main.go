...
const (
	banner = "   ____   ____ _ _____ (_)____ \n" +
		...
		"    @github.com/fasnow"
	usage = `  -t string                                      
        target http://host:port/ with no path    
  -u string                                      
        nacos username
  -p string                                      
        nacos password 
...
func (c *FlagConfig) ParseFlags() {
	fmt.Println(banner)
	flag.StringVar(&c.target, "t", "", "target http://host:port/ with no path")
	...
	if proxy != "" {
		http.SetGlobalProxy(proxy)
	}
}

func execNasin() {
	client, err := http.NewNasinClient(target, user, pass)
	if err != nil {
		fmt.Println(err)
		return
	}
